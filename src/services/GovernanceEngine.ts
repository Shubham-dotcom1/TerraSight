...
export const governanceEngine = {
    ...
    sendLegalNotice: async (detectionId: string | number) => {
        ...
        try {
            ...
            let officerEmail = localStorage.getItem('juniorEmail')?.trim();
            if (!officerEmail) officerEmail = 'officer@terrasight.com';
            ...
            if (serviceId && templateId && publicKey) {
                ...
                await emailjs.send(
                    ...
                    templateId,
                    {
                        ...
                        email_to: officerEmail,
                        email: officerEmail,
                        user_email: officerEmail,
                        to_name: officerEmail,
                        to: officerEmail,
    ...
    escalateToMinistry: async (detectionId: string | number) => {
        ...
        try {
            let seniorEmail = localStorage.getItem('seniorEmail')?.trim();
            if (!seniorEmail) seniorEmail = 'admin@ministry.gov';
            ...
            if (serviceId && templateId && publicKey) {
                await emailjs.send(
                    ...
                    templateId,
                    {
                        ...
                        email_to: seniorEmail,
                        user_email: seniorEmail,
                        email: seniorEmail,
                        to: seniorEmail,
                        to_name: seniorEmail,
                        recipient: seniorEmail,
